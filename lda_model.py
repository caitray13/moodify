...
class LdaModel():
    def __init__(self, recordCollection):
        ...
        self.lyrics_corpus = self.__build_lyrics_corpus(recordCollection.tracks)        


    def __build_lyrics_corpus(self, tracks) -> None:
        lyrics_corpus = []
        for track in tracks:
    ...
    def __build_model(self, corpus_transformed):
    ...
    def __get_top_n_similar_tracks(self, similarity_matrix, track_name, artist_name, num):
    ...
    def get_recommendations(self, track_name, artist_name, num_tracks):
        corpus_transformed = self.__vectorize(self.lyrics_corpus)
        similarity_matrix = self.__build_model(corpus_transformed)
        recommendations = self.__get_top_n_similar_tracks(similarity_matrix,
                                                          track_name,
                                                          artist_name,
                                                          num_tracks)
